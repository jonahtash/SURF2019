...
cur.execute('CREATE TABLE Terms (full_normalized text, original text, snippet text, sentence text, ID text)')
cur.execute('CREATE TABLE FragmentsLT3 (randr_fragment text, original text, ID text)')
cur.execute('CREATE TABLE TypeSearchTerms (randr_fragment text, randr_fragment_broken text, term_randr_found text, sentence_randr_found text, type INTEGER, ID text)')
...
def _make_like_inter(l):
    return ("SELECT DISTINCT sentence, ID FROM Terms WHERE full_normalized LIKE '%:" + (":%' INTERSECT SELECT DISTINCT sentence, ID FROM Terms WHERE full_normalized LIKE '%:").join(l) + ":%'")
...
def populate_terms(in_csv):
    for row in csv.reader(open(in_csv, 'r')):
        cur.execute('INSERT INTO Terms VALUES (?, ?, ?, ?)', (row[8], row[7], row[9], row[10]))
...
def make_type_1():
    ...
    for row in cur.fetchall():
        ...
        cur.execute('INSERT INTO TypeSearchTerms SELECT ?, ?, full_normalized, sentence, 1, ID FROM Terms WHERE '+_make_like(pieces), (row[0], ' '.join(pieces)))
...
def make_type_2():
    ...
    for row in cur.fetchall():
        pieces = re.split(r':[1-9][0-9]*:', row[0])
        pieces = list(set(pieces))
        statement = _make_like_inter(pieces)
        cur.execute(statement)
        for ID in cur.fetchall():
            cur.execute('INSERT INTO TypeSearchTerms VALUES (?, ?, ?, ?, ?, ?)', (row[0], ' '.join(pieces), '', ID[0], 2, ID[1]))
...
def make_type_3():
    ...
    for row in cur.fetchall():
        ...
        cur.execute(statement)
        for ID in cur.fetchall():
            cur.execute('INSERT INTO TypeSearchTerms VALUES (?, ?, ?, ?, ?, ?)', (row[0], ' '.join(pieces), '', '', 3, ID[0]))
...
if __name__ == '__main__':
    new_terms_program('terms_small.csv', 'type_zero_small.csv', 'types_all_small.csv')
