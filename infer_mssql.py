...
def populate_fragments(in_term_table,frag_table , new_term_table):
    ...
    cur.execute("if not exists (select * from sysobjects where name='"+frag_table+"' and xtype='U') create table "+frag_table+" (randr_fragment varchar(450), original varchar(450), ID varchar(450), snippet varchar(900))")
    cur.execute("if not exists (select * from sysobjects where name='"+new_term_table+"' and xtype='U') create table "+new_term_table+
