...
def populate_fragments(in_term_table,frag_table , new_term_table):
    ...
    cur.execute("if not exists (select * from sysobjects where name='"+frag_table+"' and xtype='U') create table "+frag_table+" (randr_fragment varchar(max), original varchar(max), ID varchar(max))")
    cur.execute("if not exists (select * from sysobjects where name='"+new_term_table+"' and xtype='U') create table "+new_term_table+" (randr_fragment varchar(max), randr_fragment_broken varchar(max), term_randr_found varchar(max), type INTEGER, ID varchar(max))")
